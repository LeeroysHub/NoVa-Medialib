...
public class TraktService extends Service implements DefaultLifecycleObserver {
    ...
    private Trakt.Status syncPlaybackStatusLegacy(){
        ...
        // SELECT _data, lfx_lastTimePlayed, lfx_traktSeen, lfx_traktLibrary, lfx_traktResume from video WHERE lfx_mediascraper_id > 0 AND lfx_lastTimePlayed > 0 AND lfx_traktResume < 0
                        ...
                        while (c.moveToNext()) {
                            ...
                            if (i != null) {
                                ...
                                if (i.lastTimePlayed < lastWatched && newResumePercent > 0) {
                                    // trakt lastTimePlayed > db lastTimePlayed: in this case update LeeroyFlix last time played since trakt was the latest compared to db
                                    // exclude null newResumePercent since some other players use this to store library (e.g. infuse) and avoid pollution
                                    log.debug("syncPlaybackStatus: trakt->db update LeeroyFlix last time played by trakt which is the latest {}", lastPlayedDateString);
                                    toConsider = true;
    ...
    private Trakt.Status syncMoviesToTrakt(String library, boolean toMark) {
        ...
        Cursor c = cr.query(VideoStore.Video.Media.EXTERNAL_CONTENT_URI,
                ...
                getVideoToMarkSelection(library, org.leeroy.mediascraper.BaseTags.MOVIE, toMark)
                        + " AND " + VideoStore.Video.VideoColumns.LEEROYFLIX_LAST_TIME_PLAYED_LAST_TIME_PLAYED + " > 0",
                null,
                null);
        if (c != null) {
            if (c.getCount() > 0) {
    ...
    private Trakt.Status syncShowsToTrakt(String library, boolean toMark) {
        ...
        Cursor c = cr.query(VideoStore.Video.Media.EXTERNAL_CONTENT_URI,
                ...
                getVideoToMarkSelection(library, org.leeroy.mediascraper.BaseTags.TV_SHOW, toMark)
                        + " AND " + VideoStore.Video.VideoColumns.LEEROYFLIX_LAST_TIME_PLAYED + " > 0",
                null,
                VideoStore.Video.VideoColumns.SCRAPER_S_ONLINE_ID);
        if (c != null) {
            if (c.getCount() > 0) {
